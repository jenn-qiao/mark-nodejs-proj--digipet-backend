...
app.get("/digipet/ignore", (req, res) => {
    ...
    if (digipet === null) {
        res.json({
            message: "You don't have a digipet! Try hatching one with /digipet/hatch",
        });
    }
    else if (digipet.happiness > 10 && digipet.nutrition > 10 && digipet.discipline > 10) {
        controller_1.ignoreDigipet();
        res.json({
            message: `We have ignored your digipet, all stats were decreased 10 by now`,
            digipet: model_1.getDigipet()
        });
    }
    else if (digipet.happiness <= 10 && digipet.nutrition <= 10 && digipet.discipline <= 10) {
        controller_1.ignoreDigipet();
        res.json({
            message: `We have ignored your digipet, all stats were decreased 10 by now and might be 0`,
            digipet: model_1.getDigipet(),
        });
    }
    ;
